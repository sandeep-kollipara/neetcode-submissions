...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        function<TreeNode*(vector<int>, vector<int>)> recursive_tree_traversal=[&](vector<int> preorder_arr, vector<int> inorder_arr){
            ...
            if (delta > 0 and delta < inorder_arr.size()-1){
                ...
                preorder_left = {};
                preorder_right = vector<int>(preorder_arr.begin()+1, preorder_arr.end());
                inorder_left = {};
                inorder_right = vector<int>(inorder_arr.begin()+1, inorder_arr.end());
            }
            else if (delta==inorder_arr.size()-1){
                preorder_left = vector<int>(preorder_arr.begin()+1, preorder_arr.end());
                preorder_right = {};
                inorder_left = vector<int>(inorder_arr.begin(), inorder_arr.end()-1);
                inorder_right = {};
            }
            else preorder_left = preorder_right = inorder_left = inorder_right = {};
